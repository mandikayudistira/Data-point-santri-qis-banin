...
const formSchema = z.object({
  ...
  masterId: z.coerce.number().min(1, 'Pilih pelanggaran'),
...
export default function InputPelanggaranPage() {
  ...
  const { data: masters, isLoading: mastersLoading } = useListMasterPelanggaran();
    ...
    try {
      await createMutation.mutateAsync({
        data: {
          ...
          tipe: RiwayatPoinInputTipe.pelanggaran,
  ...
  return (
    <div className="max-w-2xl mx-auto flex flex-col gap-6">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-muted-foreground text-sm">Catat pelanggaran santri baru. Poin akan otomatis mengurangi total poin santri.</p>
      </div>
      ...
      {success && (
        <div className="bg-primary/10 border border-primary/20 text-primary p-4 rounded-xl flex items-center gap-3 animate-in fade-in slide-in-from-top-2">
          ...
          <p className="text-sm font-medium">Pelanggaran berhasil dicatat.</p>
        </div>
      )}
      ...
      <div className="bg-card border border-border rounded-xl shadow-sm p-6">
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5">
          ...
          <div className="space-y-2">
            <label className="text-sm font-semibold text-foreground">
              Pelanggaran <span className="text-destructive">*</span>
            </label>
            <select
              {...register('masterId')}
              className="flex h-11 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
            ...
            {selectedMaster && (
              <div className="mt-2 p-3 bg-destructive/5 rounded-lg border border-destructive/10 text-sm flex justify-between">
                <span className="text-muted-foreground">Poin yang akan dikurangi:</span>
                <span className="font-bold font-mono text-destructive">{selectedMaster.poin}</span>
              </div>
            )}
          </div>
          ...
          <div className="pt-4 border-t border-border flex justify-end gap-3">
            ...
            <button
              ...
              className="px-5 py-2.5 rounded-lg text-sm font-medium text-destructive-foreground bg-destructive hover:bg-destructive/90 transition-colors disabled:opacity-50"
            >
